<x-layouts.admin title="Edit Match">
    <h1 class="mb-6 text-2xl font-bold">Edit Match #{{ $match->id }}</h1>
    ...
    @php
        $streams = old('streams', $match->streams->map(fn($stream) => [
        ...
        if (count($streams) === 0) {
            $streams = [['channel_name' => '', 'iframe_url' => '', 'stream_type' => 'iframe']];
        }
    @endphp

    <form method="POST" action="{{ route('admin.matches.update', $match->id) }}" class="space-y-5 rounded-xl border border-zinc-800 bg-[#12121e] p-6">
        @csrf
        @method('PUT')
        <div class="grid gap-4 md:grid-cols-2">
            <div class="md:col-span-2">
                <label class="mb-1 block text-sm">Title</label>
                <input type="text" name="title" value="{{ old('title', $match->title) }}" required class="w-full rounded border border-zinc-700 bg-zinc-900 px-3 py-2">
            </div>
            <div>
                <label class="mb-1 block text-sm">League</label>
                <input type="text" name="league" value="{{ old('league', $match->league) }}" class="w-full rounded border border-zinc-700 bg-zinc-900 px-3 py-2">
            </div>
            <div>
                <label class="mb-1 block text-sm">Country</label>
                <input type="text" name="country" value="{{ old('country', $match->country) }}" class="w-full rounded border border-zinc-700 bg-zinc-900 px-3 py-2">
            </div>
            <div>
                <label class="mb-1 block text-sm">Team Home</label>
                <input type="text" name="team_home" value="{{ old('team_home', $match->team_home) }}" class="w-full rounded border border-zinc-700 bg-zinc-900 px-3 py-2">
            </div>
            <div>
                <label class="mb-1 block text-sm">Team Away</label>
                <input type="text" name="team_away" value="{{ old('team_away', $match->team_away) }}" class="w-full rounded border border-zinc-700 bg-zinc-900 px-3 py-2">
            </div>
            <div>
                <label class="mb-1 block text-sm">Match Date</label>
                <input type="date" name="match_date" value="{{ old('match_date', optional($match->match_datetime)->format('Y-m-d')) }}" required class="w-full rounded border border-zinc-700 bg-zinc-900 px-3 py-2">
            </div>
            <div>
                <label class="mb-1 block text-sm">Match Time</label>
                <input type="time" name="match_time" value="{{ old('match_time', optional($match->match_datetime)->format('H:i')) }}" required class="w-full rounded border border-zinc-700 bg-zinc-900 px-3 py-2">
            </div>
            <div>
                ...
                <select name="server_id" required class="w-full rounded border border-zinc-700 bg-zinc-900 px-3 py-2">
                    ...
                    @foreach($servers as $server)
                        <option value="{{ $server->id }}" @selected(old('server_id', $match->server_id) == $server->id)>{{ $server->name }}</option>
                    @endforeach
                </select>
            </div>
            <div class="flex items-center gap-2 pt-6">
                <input type="checkbox" id="is_featured" name="is_featured" value="1" @checked(old('is_featured', $match->is_featured))>
        ...
        <button type="submit" class="rounded-lg bg-[#e94560] px-5 py-2.5 font-semibold">Save Changes</button>
    </form>
